...
@end
#elif __IPHONE_OS_VERSION_MIN_REQUIRED == __IPHONE_14_0
typedef NS_ENUM(int, StatusBarItem) {
  TimeStatusBarItem = 0,
  DateStatusBarItem = 1,
  QuietModeStatusBarItem = 2,
...
NSString* getCarrierOverride(void) {
    StatusBarOverrideData *overrides = [UIStatusBarServer getStatusBarOverrideData];
    NSString *carrier = [NSString stringWithFormat:@"%s", overrides->values.serviceString];
    return carrier;
}
...
NSString* getTimeOverride(void) {
    StatusBarOverrideData *overrides = [UIStatusBarServer getStatusBarOverrideData];
    NSString *time = [NSString stringWithFormat:@"%s", overrides->values.timeString];
    return time;
}
...
bool isWiFiHidden(void) {
    StatusBarOverrideData *overrides = [UIStatusBarServer getStatusBarOverrideData];
    return overrides->overrideItemIsEnabled[CellularSignalStrengthStatusBarItem] == 1;
}
